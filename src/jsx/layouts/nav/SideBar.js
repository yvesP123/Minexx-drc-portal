...
const SideBar = ({ language, country }) => {
  ...
  // Process menu items - Simplified for DRC only
                    ...
                    <Collapse in={state.active === data.title}>
                      <ul className={`${menuClass === "mm-collapse" && data.content ? "mm-show" : ""}`}>
                        {data.content.filter(c => {
                          ...
                          if (access === `gold`) {
                            return !["reports/daily", "reports/mtd", "reports/deliveries", "reports/sale", "reports/suppliertrends","reports/suppliercomposition","reports/deliverygradetrends","reports/exportchemicalcomposition"].includes(c.to);
                          }
                          return true;
