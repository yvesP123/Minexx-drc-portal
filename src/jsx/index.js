...
const Markup = (props) => {
  ...
  // Get initial country (always DRC)
  const getInitialCountry = () => {
    return localStorage.getItem(`_country`) || 'DRC';
  };
  
  const [language, setLanguage] = useState(localStorage.getItem('_lang') || 'fr');
  const [country, setCountry] = useState(getInitialCountry());

  // Ensure country is always DRC
  useEffect(() => {
    const currentCountry = localStorage.getItem('_country');
    if (currentCountry !== 'DRC') {
      setCountry('DRC');
