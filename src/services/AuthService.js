...
export function login(email, password) {
    ...
    return axios.post( 
        `https://minexxapi-togo-clone-p7n5ing2cq-uc.a.run.app/login`,
        postData,
    );
} 
...
export function saveTokenInLocalStorage(data) {
    ...
    // Set country to DRC only
    const initialCountry = 'DRC';
    localStorage.setItem('_country', initialCountry);
    
    // Set language based on country (DRC defaults to French)
    ...
    if (!userLang) {
        localStorage.setItem('_lang', countryLanguageDefaults[initialCountry] || 'fr');
    }
}
